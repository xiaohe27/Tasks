	...
	final void event_hasnext(Iterator i) {
		...
		for(int i_1 = 0; i_1 < this.size; i_1++){
			...
			if(!monitor.isTerminated()){
				...
				final HasNextMonitor monitorfinalMonitor = monitor;
				monitor.Prop_2_event_hasnext(i);
				if(monitorfinalMonitor.Prop_2_Category_match) {
					monitorfinalMonitor.Prop_2_handler_match();
				}
			}
		}
...
class HasNextMonitor extends com.runtimeverification.rvmonitor.java.rt.tablebase.AbstractAtomicMonitor implements Cloneable, com.runtimeverification.rvmonitor.java.rt.RVMObject {
	...
	static final int Prop_2_transition_hasnext[] = {2, 2, 2, 3};;
	static final int Prop_2_transition_next[] = {1, 1, 0, 3};;

	volatile boolean Prop_1_Category_fail = false;
	volatile boolean Prop_2_Category_match = false;
	...
	final boolean Prop_2_event_hasnext(Iterator i) {
		{}

		int nextstate = this.handleEvent(0, Prop_2_transition_hasnext) ;
		this.Prop_2_Category_match = nextstate == 1;

		return true;
	}

	final boolean Prop_2_event_next(Iterator i) {
		{}

		int nextstate = this.handleEvent(1, Prop_2_transition_next) ;
		this.Prop_2_Category_match = nextstate == 1;
	...
	final void Prop_2_handler_match (){
		{
			System.out.println("next called without hasNext!");
		}
...
public final class HasNext3RuntimeMonitor implements com.runtimeverification.rvmonitor.java.rt.RVMObject {
	private static com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager HasNext3MapManager;
	static {
		HasNext3MapManager = new com.runtimeverification.rvmonitor.java.rt.map.RVMMapManager();
		HasNext3MapManager.start();
	}

	// Declarations for the Lock
	static final ReentrantLock HasNext3_RVMLock = new ReentrantLock();
	static final Condition HasNext3_RVMLock_cond = HasNext3_RVMLock.newCondition();
	...
	public static final void hasnextEvent(Iterator i) {
		...
		while (!HasNext3_RVMLock.tryLock()) {
			Thread.yield();
		}
		...
		final HasNextMonitor matchedEntryfinalMonitor = matchedEntry;
		matchedEntry.Prop_2_event_hasnext(i);
		if(matchedEntryfinalMonitor.Prop_2_Category_match) {
			matchedEntryfinalMonitor.Prop_2_handler_match();
		}
	...
	public static final void nextEvent(Iterator i) {
		...
		final HasNextMonitor matchedEntryfinalMonitor = matchedEntry;
		matchedEntry.Prop_2_event_next(i);
		if(matchedEntryfinalMonitor.Prop_2_Category_match) {
			matchedEntryfinalMonitor.Prop_2_handler_match();
		}
