...
{
this in footprint 
&& spine <= footprint - {this}
&& head != null 
&& head in spine 
&&
(forall nd :: nd in spine ==> (
nd != null && 
nd.footprint <= footprint - {this}
&& nd.Valid()
) &&
(head in nd.footprint ==> head == nd)
)
&&
...
assert head.next != null ==> newNd !in head.next.footprint && head.next in spine;
...
assert (forall nd :: nd in spine ==> (nd.next != null ==> nd.next in spine));

head.next := newNd;
head.footprint := head.footprint + {newNd};
head.tailContents := [d] + head.tailContents;

assert head !in newNd.footprint;
assert newNd.Valid();
assert head.Valid();
assert forall nd :: nd in spine ==> nd != null && nd.Valid();
