...
assert curNd.next == null && curNd == spine[|spine|-1];

assert forall nd :: nd in spine ==> nd != null && nd.good();
assert forall nd :: nd in spine ==> nd.footprint <= head.footprint;

curNd.next := newNd;

assert newNd.next == null && newNd.Valid(); //5s

assert forall nd :: nd in spine ==> newNd.footprint !! nd.footprint; //2s

assert goodSeqCond(spine);

assert forall node :: node in spine[0..|spine|-1] ==> 
    nodeNoChangeStillGood(old(node), old(node.next),
	old(node.tailContents),
	old(node.footprint),
	old(node.next.tailContents),
	old(node.next.footprint),
	old(node.next.data), node);

//assert forall node :: node in spine[0..|spine|-1] ==> node.good();

assert curNd == spine[|spine|-1] &&
	curNd.next == newNd && curNd.tailContents == []
	&& curNd.footprint == {curNd};


assert forall nd :: nd in spine ==> nd.next != null;

//above ok, no ghost method mkSpineValid: 2min21s, 2min11s

assert spine != [] &&
goodSeqCond(spine) &&
forall nd :: nd in spine[0..|spine|-1] ==> nd.good() &&
forall nd :: nd in spine ==> nd.next != null &&
newNd != null && newNd.next == null 
    && newNd.Valid() && forall nd :: nd in spine ==>
		newNd.footprint !! nd.footprint &&
	 spine[|spine|-1].next == newNd &&
         spine[|spine|-1].tailContents == [] &&
	 spine[|spine|-1].footprint == {spine[|spine|-1]};

//spine := mkSpineValid(spine, newNd);
...
assert forall nd :: nd in spine[0..|spine|-1] ==> nd.good();

assert spine[|spine|-1].next == newNd &&
         spine[|spine|-1].tailContents == [newNd.data] &&
	 spine[|spine|-1].footprint == {spine[|spine|-1], newNd};

assert spine[|spine|-1].Valid();
goodValidLemma(spine, |spine|-1);
